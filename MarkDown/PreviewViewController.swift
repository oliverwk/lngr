...
class PreviewViewController: UIViewController, QLPreviewingController, WKNavigationDelegate {
    ...
    func preparePreviewOfFile(at url: URL, completionHandler handler: @escaping (Error?) -> Void) {
        ...
        do {
            ...
            <html><head><meta name=\"viewport\" content=\"width=device-width, initial-scale=1\"><style>* { font-family: -apple-system, BlinkMacSystemFont, \"Segoe UI\", Helvetica, Arial, sans-serif, \"Apple Color Emoji\", \"Segoe UI Emoji\"; }</style></head><body><h1>SnapKit-LoginKit</h1>
                <p>This example will show how to use snapchat's LoginKit with swiftui.</p>
                <p>An API that changed in IOS 14 was how you handle url's as you see below this is the old way of doing it</p>
                <pre><code class="language-swift">import SCSDKLoginKit
                ...
                <p>While the new of doing things is this wich as you can see does not pass a <code>UIApplication</code> only a url as you can see below.</p>
                <pre><code class="language-swift">ConentView()
                    .onOpenURL(perform: { url in
                        print(ulr)
                    })
                </code></pre>
                <p>So you need to pass it <code>UIApplication.shared</code> which does the trick.</p>
                <pre><code class="language-swift">ConentView()
                    .onOpenURL(perform: { url in
